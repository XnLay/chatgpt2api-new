...
class YydsMailProvider(BaseMailProvider):
    ...
    def __init__(self, entry: dict, conf: dict):
        super().__init__(conf, str(entry.get("provider_ref") or ""))
        self.api_base = str(entry.get("api_base") or "https://tempmail.uxwi.de").rstrip("/")
    ...
    def _request(self, method: str, path: str, params: dict | None = None, payload: dict | None = None, expected: tuple[int, ...] = (200, 201, 204)):
    ...
    def create_mailbox(self, username: str | None = None) -> dict[str, Any]:
        payload = {"prefix": username or _random_mailbox_name()}
        if self.domain:
            payload["domain"] = _next_domain(self.domain)
        if self.wildcard or self.subdomain:
            payload["address_type"] = "subdomain"
        else:
            payload["address_type"] = "root"
        if self.subdomain:
            payload["subdomain"] = self.subdomain
        data = self._request("POST", "/api/generate-email", payload=payload, expected=(200, 201))
        # HLOOL Mail 的创建接口只返回邮箱地址，后续请求继续使用配置的 API Key。
        address = str(data.get("email") or data.get("address") or "").strip()
        if not address:
            raise RuntimeError("YYDSMail 缺少 email/address")
        return {"provider": self.name, "provider_ref": self.provider_ref, "address": address, "account_id": str(data.get("id") or "")}

    def fetch_latest_message(self, mailbox: dict[str, Any]) -> dict[str, Any] | None:
        data = self._request("GET", "/api/emails/next", params={"email": mailbox["address"]})
        ...
        text_content = str(item.get("text_content") or item.get("text") or "")
        ...
        if isinstance(sender, dict):
            sender = sender.get("address") or sender.get("email") or sender.get("name") or ""
        return {"provider": self.name, "mailbox": mailbox["address"], "message_id": message_id, "subject": str(item.get("subject") or ""), "sender": str(sender), "text_content": text_content, "html_content": html_content, "received_at": _parse_received_at(item.get("created_at") or item.get("createdAt") or item.get("received_at") or item.get("receivedAt") or item.get("date") or item.get("timestamp")), "raw": item}
